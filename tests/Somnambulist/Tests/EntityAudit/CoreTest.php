...
use Somnambulist\EntityAudit\Tests\Fixtures\Core\ArticleAudit;
use Somnambulist\EntityAudit\Tests\Fixtures\Core\Cat;
use Somnambulist\EntityAudit\Tests\Fixtures\Core\Dog;
use Somnambulist\EntityAudit\Tests\Fixtures\Core\Fox;
use Somnambulist\EntityAudit\Tests\Fixtures\Core\Rabbit;
 ...
 * @subpackage Somnambulist\EntityAudit\Tests\CoreTest
 */
class CoreTest extends BaseTest
{
    ...
    public function testFindNoRevisionFound()
    {
        ...
        $auditUser = $reader->find('Somnambulist\EntityAudit\Tests\Fixtures\Core\UserAudit', 1, 1);
    }
    ...
    public function testFindRevisionHistory()
    {
        ...
        $this->assertContainsOnly('Somnambulist\EntityAudit\Revision', $revisions);

        $this->assertEquals(2, $revisions[0]->getRev());
        $this->assertInstanceOf('DateTimeInterface', $revisions[0]->getTimestamp());
        $this->assertEquals('beberlei', $revisions[0]->getUsername());

        $this->assertEquals(1, $revisions[1]->getRev());
        $this->assertInstanceOf('DateTimeInterface', $revisions[1]->getTimestamp());
    ...
    public function testFindEntitesChangedAtRevision()
    {
        ...
        $this->assertEquals('Somnambulist\EntityAudit\Tests\Fixtures\Core\ArticleAudit', $changedEntities[0]->getClassName());
        $this->assertEquals('INS', $changedEntities[0]->getRevisionType());
        $this->assertEquals(['id' => 1], $changedEntities[0]->getId());
        $this->assertInstanceOf('Somnambulist\EntityAudit\Tests\Fixtures\Core\ArticleAudit', $changedEntities[0]->getEntity());

        $this->assertEquals('Somnambulist\EntityAudit\Tests\Fixtures\Core\UserAudit', $changedEntities[1]->getClassName());
        $this->assertEquals('INS', $changedEntities[1]->getRevisionType());
        $this->assertEquals(['id' => 1], $changedEntities[1]->getId());
        $this->assertInstanceOf('Somnambulist\EntityAudit\Tests\Fixtures\Core\UserAudit', $changedEntities[1]->getEntity());
    }
    ...
    public function testDeleteUnInitProxy()
    {
        ...
        $user = $this->em->getReference("Somnambulist\\EntityAudit\\Tests\\Fixtures\\Core\\UserAudit", 1);
        $this->em->remove($user);
        $this->em->flush();
        ...
        $this->assertContainsOnly('Somnambulist\EntityAudit\ChangedEntity', $changedEntities);
        $this->assertEquals('Somnambulist\EntityAudit\Tests\Fixtures\Core\UserAudit', $changedEntities[0]->getClassName());
