...
use Somnambulist\EntityAudit\Collection\AuditedCollection;
use Somnambulist\EntityAudit\Exception\DeletedException;
use Somnambulist\EntityAudit\Exception\InvalidRevisionException;
use Somnambulist\EntityAudit\Exception\NoRevisionFoundException;
use Somnambulist\EntityAudit\Exception\NotAuditedException;
use Somnambulist\EntityAudit\Metadata\MetadataFactory;
use Somnambulist\EntityAudit\Utils\ArrayDiff;

/**
 * Class AuditReader
 *
 * @package    Somnambulist\EntityAudit
 * @subpackage Somnambulist\EntityAudit\AuditReader
 */
class AuditReader
{
    /**
    ...
    public function findRevisionHistory($limit = 20, $offset = 0)
    {
        ...
        foreach ($revisionsData AS $row) {
            $revisions[] = new Revision(
                $row['id'],
                \DateTimeImmutable::createFromFormat($this->platform->getDateTimeFormatString(), $row['timestamp']),
                $row['username']
            );
        }
